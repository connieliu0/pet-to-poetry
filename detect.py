...
def main(type, iou_threshold, confidence_threshold, input_names):
    ...
    if type == 'images':
        ...
        inputs = tf.compat.v1.placeholder(
            tf.float32, [batch_size, *_MODEL_SIZE, 3])
        detections = model(inputs=inputs, training=False)
        ...
        res = draw_boxes(input_names, detection_result,
...
#     main(type=sys.argv[1], iou_threshold=float(sys.argv[2]), confidence_threshold=float(sys.argv[3]), input_names=sys.argv[4:])
